...
export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "eeroNotebook APIサーバーに到達できませんでした",
  ...
  notebooks: {
    ...
    lastViewed: "{{time}}に表示",
    share: "共有",
    shareNotebook: "ノートブックを共有",
    shareNotebookDesc: "「{{name}}」への読み取り専用アクセスを1人に付与します。ソースを閲覧して質問できますが、何も変更できません。",
    shareEmailLabel: "メールアドレス",
    shareEmailPlaceholder: "person@example.com",
    shareGrant: "アクセスを付与",
    shareOnePersonHint: "一度に1人だけです。ノートブックを全員に共有する方法はありません。",
    shareLoading: "アクセス権を持つメンバーを読み込み中...",
    shareNobodyYet: "このノートブックにアクセスできる他のメンバーはいません。",
    shareRoleViewer: "閲覧者",
    shareGrantedAgo: "付与: {{time}}",
    shareUnnamedMember: "メールアドレスのないメンバー",
    shareRevoke: "アクセスを取り消す",
    shareRevokeConfirm: "{{name}} のこのノートブックへのアクセスを終了しますか？アクセスは直ちに停止し、ノートブックの内容は削除されません。",
    shareGrantSuccess: "アクセスを付与しました",
    shareGrantFailed: "ノートブックを共有できませんでした",
    shareRevokeSuccess: "アクセスを取り消しました",
    shareRevokeFailed: "アクセスを取り消せませんでした",
    sharedWithYou: "あなたに共有されています",
    viewerReadOnlyNotice: "このノートブックへのアクセスは読み取り専用です。ソースを閲覧して質問できます。",
    noNotesViewer: "このノートブックの所有者はまだメモを追加していません。",
  },
  sources: {
    bulkContext: "コンテキスト",
    ...
    noSourcesViewer: "このノートブックの所有者はまだソースを追加していません。",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはeeroNotebookがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "eeroNotebook全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {{models}}。これらがないとeeroNotebookが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、AIプロバイダーをeeroNotebookで利用可能にします。",
