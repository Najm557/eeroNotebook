...
- [-] 5. Identity
  ...
  - [-] 5.4 Move the frontend onto member sessions — **API side done, browser side outstanding**
    ...
    - **Outstanding — the browser half:** rewrite `frontend/src/app/(auth)/login/` and `LoginForm` to collect an email as well as a password and post to `/api/auth/login`; extend `auth-store` to hold the refresh token and expiry and refresh ahead of expiry; add sign-out and a signed-in indicator; i18n keys in every locale; `npm run lint`, `test`, `build`
    - **Outstanding — and not verifiable from here:** the end-to-end browser flow needs `eeronotebook.local` to resolve and the private CA to be trusted, both of which need `sudo` on the Dev Server
